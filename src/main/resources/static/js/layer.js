...
mozip.popup = {
    ...
    open(url, title, width, height, isIframe) {
        ...
        layerDim.addEventListener("click", mozip.popup.close);
        ...
        if (isIframe) {
            ...
            if (xhr.readyState == XMLHttpRequest.DONE && xhr.status == 200) {
                ...
                if (typeof mozip.popup.callback == 'function') {
                    typeof mozip.popup.callback();
                }
            }
            };
            xhr.send(null);
        ...
        layerClose.addEventListener("click", mozip.popup.close);
        /** 팝업 닫기 버튼 E */
...
window.addEventListener("DOMContentLoaded", function() {
    ...
    if (layerPopups.length > 0) {
        for (var i = 0; i < layerPopups.length; i++) {
            layerPopups[i].addEventListener("click", function(e) {
                ...
                mozip.popup.open(url, title, Number(width), Number(height));
            });
    ...
    if (layerCloses.length > 0) {
        for (var i = 0; i < layerCloses.length; i++) {
            layerCloses[i].addEventListener("click", mozip.popup.close);
        }
    }
    /** 답업 닫기 처리 E */
});